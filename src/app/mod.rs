...
impl App {
    ...
    pub fn save_profile(&mut self) {
        let (name, comp_idx, sched_idx) = if let AppState::CreatingProfile(ref wizard) = self.state {
            (wizard.name.clone(), wizard.compression_idx, wizard.schedule_idx)
        } else {
            return;
        };
        ...
        let compression = crate::ui::profiles::COMPRESSION_OPTIONS[comp_idx].to_string();
        let schedule = crate::ui::profiles::SCHEDULE_OPTIONS[sched_idx].0.to_string();

        let new_profile = crate::config::BackupProfile {
            id: format!(
                "prof-{}",
            ...
            name: trimmed_name,
            paths: self.file_browser.explicit_includes.iter().cloned().collect(),
            excludes: self.file_browser.explicit_excludes.iter().cloned().collect(),
            counter: 0,
            compression,
            schedule,
        };
    ...
    pub fn run_profile_backup(&mut self, profile_idx: usize) {
        let (name, paths, excludes, compression) = if let Some(p) = self.config.profiles.get_mut(profile_idx) {
            let archive_name = p.next_archive_name(chrono::Local::now());
            p.counter += 1;
            let inc: Vec<String> = p.paths.iter().map(|path| path.to_string_lossy().to_string()).collect();
            let exc: Vec<String> = p.excludes.iter().map(|path| path.to_string_lossy().to_string()).collect();
    ...
    pub fn delete_profile(&mut self, profile_idx: usize) {
        if profile_idx < self.config.profiles.len() {
            ...
            self.state = AppState::ManagingProfiles { selected_index: next_idx };
        }
    }

    pub fn install_systemd_profile(&mut self, profile: &crate::config::BackupProfile) {
        let (repo_path, passphrase) = match self.get_active_repo() {
            Some(r) => (r.location.clone(), r.passphrase.clone()),
            None => (config::get_default_repo_path(), None),
        };
...
mod tests {
    ...
    fn test_profile_creation_and_run() {
        ...
        app.file_browser.explicit_includes.insert(PathBuf::from("/tmp/test"));
        app.save_profile();
